...
# execute the given question queries
...
# channel data through YouTube API
def fetch_channel_data(new_channel_id):
    try:
        ...
        # Relevant channel data 
            ...
            # put channel data into mysql database
            mydb = mysql.connector.connect(host="localhost", user="root", password="", database="youtube")
            cursor = mydb.cursor()
            cursor.execute("""
                INSERT INTO channeldetails (channel_id, channel_name, channel_des, channel_playid, channel_vidcount, channel_viewcount, channel_subcount)
...
#  video data using through YouTube API
    ...
    for new_channel_id in channel_ids:
        ...
        try:
            ...
            if 'items' in response and len(response['items']) > 0:
                ...
                st.error(f"No channeldetails found for ID: {new_channel_id}")
        except HttpError as e:
            st.error(f"HTTP Error: {e}")
        except KeyError as e:
...
# to join video data using YouTube API
def fetch_video_data(all_video_ids):
    ...
    # put video data into mysql database
    mydb = mysql.connector.connect(host="localhost", user="root", password="", database="youtube")
    cursor = mydb.cursor()
...
# comment data using YouTube API
def fetch_comment_data(new_channel_id):
    ...
    # put comment data into mysql database
    mydb = mysql.connector.connect(host="localhost", user="root", password="", database="youtube")
    cursor = mydb.cursor()
...
def main():
    ...
    selected_option = st.sidebar.radio("Select Option", ("Channeldetails", "Videos", "Comments", "Queries", "Enter YouTube Channel ID"))

    if selected_option == "Channeldetails":
        st.header("Channeldetails")
        channel_df = fetch_data("SELECT * FROM channeldetails;")
        channel_df.index += 1
        ...
        if st.button("Fetch Channel Data"):
            channels_data_df = fetch_channel_data(channel_id)
            channels_data_df.index+=1
